...
func TestOrAndGroupPredicates(t *testing.T) {
	...
	objectTests := []struct {
		...
		},
		{
			...
			query:     "object:project (has:due | has:todo)",
	...
	traitTests := []struct {
	...
	}{
		{
			...
			query:     "trait:due (on:project | on:person)",
		...
		},
		{
			...
			query:     "trait:todo (value:todo) on:section",
			wantCount: 1, // trait5
		},
	}
...
func TestSortedTraitQuery(t *testing.T) {
	db := setupTestDB(t)
	defer db.Close()
	...
	tests := []struct {
		name       string
		query      string
		wantFirst  string // Expected first result's value or content
		wantCount  int
	}{
		{
			name:      "sort by value ascending",
			query:     "trait:due sort:_.value",
			wantFirst: "2025-02-01", // trait4 has earliest date
			wantCount: 3,
		},
		{
			name:      "sort by value descending",
			query:     "trait:due sort:_.value:desc",
			wantFirst: "2025-06-30", // trait1 has latest date
			wantCount: 3,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := executor.ExecuteTraitQuerySorted(q)
			if err != nil {
				t.Fatalf("unexpected error: %v", err)
			}
			...
			results := result.Groups[0].Results
			if len(results) != tt.wantCount {
				t.Errorf("got %d results, want %d", len(results), tt.wantCount)
			}
			if len(results) > 0 && results[0].Value != nil && *results[0].Value != tt.wantFirst {
				t.Errorf("first result value = %v, want %v", *results[0].Value, tt.wantFirst)
			}
		})
	}
}

func TestGroupedTraitQuery(t *testing.T) {
	db := setupTestDB(t)
	defer db.Close()
	...
	tests := []struct {
	...
	}{
		{
			...
			query:          "trait:todo group:_.parent",
			wantGroupCount: 2, // Two different parent objects
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(result.Groups) != tt.wantGroupCount {
				t.Errorf("got %d groups, want %d", len(result.Groups), tt.wantGroupCount)
				for i, g := range result.Groups {
					t.Logf("  group %d: key=%s, count=%d", i, g.Key, len(g.Results))
				}
			}
		})
	}
...
func TestSortedObjectQuery(t *testing.T) {
	db := setupTestDB(t)
	defer db.Close()
	...
	tests := []struct {
	...
	}{
		{
			...
			query:     "object:project sort:_.status",
			wantFirst: "projects/website", // active comes before paused
			wantCount: 2,
		},
		{
			name:      "sort by status descending",
			query:     "object:project sort:_.status:desc",
			wantFirst: "projects/mobile", // paused comes before active when descending
			wantCount: 2,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := executor.ExecuteObjectQuerySorted(q)
			if err != nil {
				t.Fatalf("unexpected error: %v", err)
			}
			if len(result.Groups) != 1 {
			...
			if len(results) > 0 && results[0].ID != tt.wantFirst {
				t.Errorf("first result ID = %v, want %v", results[0].ID, tt.wantFirst)
			}
		})
	}
}
...
func TestLimitClause(t *testing.T) {
	...
	// Test limit on object query
	q, err := Parse("object:project limit:1")
	if err != nil {
		t.Fatalf("parse error: %v", err)
	}
	...
	// Test limit on trait query
	q2, err := Parse("trait:due limit:2")
	if err != nil {
		t.Fatalf("parse error: %v", err)
	}

	traitResults, err := executor.ExecuteTraitQuery(q2)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(traitResults) != 2 {
		t.Errorf("got %d results, want 2", len(traitResults))
	}
}
...
func TestRefdShorthand(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "refd shorthand",
			query:     "object:project refd:meeting",
			wantCount: 2, // website referenced by standup, mobile by planning
		},
	}
...
func TestHierarchyPredicatesWithSubqueries(t *testing.T) {
	...
	objectTests := []struct {
		...
		},
		{
			...
			query:     "object:date descendant:{object:meeting has:highlight}",
		...
		},
		{
			...
			query:     "object:section parent:{object:project has:due}",
	...
	traitTests := []struct {
		...
		},
		{
			...
			query:     "trait:todo within:{object:project has:due}",
		...
		},
		{
			...
			query:     "trait:highlight within:date",
