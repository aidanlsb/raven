...
func (p *Parser) parseQuery() (*Query, error) {
	...
	// v2: Check for pipeline operator |>
	...
	// v1 compat: Parse optional sort/group/limit clauses
...
func (p *Parser) parsePredicates(qt QueryType) ([]Predicate, error) {
	...
	for {
		...
		// v1 compat: Stop at sort/group/limit keywords (these are clauses, not predicates)
		if p.curr.Type == TokenIdent {
			keyword := strings.ToLower(p.curr.Value)
