...
In each pair, the first command previews the bulk change and the second applies it after approval. Section queries support `--apply "add <text>"` (e.g. `rvn query 'section .title==Tasks' --apply 'add - review backlog' --confirm`); asset queries do not support `--apply`.
