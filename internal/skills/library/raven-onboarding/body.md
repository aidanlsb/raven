...
2. **If there is no vault yet, create one (primary path):**
   ...
   - Read `post_init` from the result and honor what init already did:
     ...
     - If `is_first_vault` is `false`, init made the new vault active. Surface `active_vault`, `previous_active_vault` / `previous_vault`, and `switch_back` before continuing. Ask before changing the default.
   ...
   - After init, work against this now-active vault; explicit `--vault <name>` or `--vault-path <path>` remains available when needed.
...
8. **Teach by doing — against their data.** Keep mechanics attached to their content instead of a separate feature tour.
   ...
   - Contrast the two write homes concretely: update a field with `rvn set <reference> --field status=... --json`; mark a line with a trait via `rvn add "@todo ..." --json`.
...
- Update a field with `rvn set <reference> --field field=value --json`; add a trait by writing a line with `rvn add "@trait ..." --json`.
