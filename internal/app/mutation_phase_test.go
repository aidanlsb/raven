...
func TestMutationPhaseSingleObjectWrites(t *testing.T) {
	...
	t.Run("trash empty previews by default", func(t *testing.T) {
		t.Parallel()
		v := buildPhaseVault(t)
		deleted := runInvoked(t, v.Path, "delete", map[string]any{"reference": "projects/scratch"}, nil)
		requirePhase(t, deleted, commandexec.MutationPhaseApplied)
		res := runInvoked(t, v.Path, "trash_empty", map[string]any{}, nil)
		requirePhase(t, res, commandexec.MutationPhasePreview)
	})

	t.Run("trash empty applies with confirm", func(t *testing.T) {
		t.Parallel()
		v := buildPhaseVault(t)
		deleted := runInvoked(t, v.Path, "delete", map[string]any{"reference": "projects/scratch"}, nil)
		requirePhase(t, deleted, commandexec.MutationPhaseApplied)
		res := runInvoked(t, v.Path, "trash_empty", map[string]any{}, withConfirm)
		requirePhase(t, res, commandexec.MutationPhaseApplied)
	})
