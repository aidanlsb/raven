...
func TestTypeTraits(t *testing.T) {
	t.Run("HasTrait", func(t *testing.T) {
		...
		if !tt.HasTrait("due") {
			t.Error("expected HasTrait('due') to be true")
		}
		if !tt.HasTrait("priority") {
			t.Error("expected HasTrait('priority') to be true")
		}
		if tt.HasTrait("nonexistent") {
			t.Error("expected HasTrait('nonexistent') to be false")
		}
	})

	t.Run("HasTrait with nil Configs", func(t *testing.T) {
		tt := TypeTraits{}
		if tt.HasTrait("due") {
			t.Error("expected HasTrait to be false when Configs is nil")
		}
	})
	...
	t.Run("IsRequired", func(t *testing.T) {
		...
		if tt.IsRequired("due") {
			t.Error("expected 'due' to not be required")
		}
		if !tt.IsRequired("priority") {
			t.Error("expected 'priority' to be required")
		}
		if tt.IsRequired("nonexistent") {
			t.Error("expected nonexistent trait to not be required")
		}
	})

	t.Run("IsRequired with nil Configs", func(t *testing.T) {
		tt := TypeTraits{}
		if tt.IsRequired("due") {
			t.Error("expected IsRequired to be false when Configs is nil")
		}
	})
	...
	t.Run("GetDefault", func(t *testing.T) {
		...
		if tt.GetDefault("due") != nil {
			t.Error("expected 'due' default to be nil")
		}
		if tt.GetDefault("priority") != "medium" {
			t.Errorf("expected 'priority' default to be 'medium', got %v", tt.GetDefault("priority"))
		}
		if tt.GetDefault("nonexistent") != nil {
			t.Error("expected nonexistent trait default to be nil")
		}
	})

	t.Run("GetDefault with nil Configs", func(t *testing.T) {
		tt := TypeTraits{}
		if tt.GetDefault("due") != nil {
			t.Error("expected GetDefault to be nil when Configs is nil")
		}
	})
	...
	t.Run("List", func(t *testing.T) {
		...
		list := tt.List()
		if len(list) != 2 {
			t.Errorf("expected 2 traits, got %d", len(list))
		}
	})

	t.Run("List with nil Configs", func(t *testing.T) {
		tt := TypeTraits{}
		list := tt.List()
		if list != nil {
			t.Error("expected List to be nil when Configs is nil")
		}
	})
}
