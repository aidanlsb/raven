		...
		if baseID, fragment, isSection := paths.ParseSectionID(ref.TargetRaw); isSection && fragment != "" {
			...
			if baseResult.TargetID != "" {
				issues = append(issues, Issue{
					...
					Value:    ref.TargetRaw,
					FixHint:  "The heading may have been renamed. Update the fragment to match an existing section slug. To rename a heading safely next time, use 'rvn section rename <file#section> \"<new heading text>\"' which rewrites inbound references.",
				})
				return issues
			}
		}
