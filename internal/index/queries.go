...
// QueryTags queries objects by tag.
func (d *Database) QueryTags(tag string) ([]TagResult, error) {
	rows, err := d.db.Query(`
		SELECT tag, object_id, file_path, line_number
		FROM tags
		WHERE tag = ?
	...
	for rows.Next() {
		...
		if err := rows.Scan(&result.Tag, &result.ObjectID, &result.FilePath, &result.Line); err != nil {
			return nil, err
		}
		results = append(results, result)
	}
...
// QueryAllTags returns all tags with their usage counts.
func (d *Database) QueryAllTags() ([]TagCount, error) {
	rows, err := d.db.Query(`
	...
	for rows.Next() {
		var result TagCount
		if err := rows.Scan(&result.Tag, &result.Count); err != nil {
			return nil, err
		}
		results = append(results, result)
	}
...
// QueryTagsMultiple queries multiple tags at once (AND logic - objects that have ALL tags).
func (d *Database) QueryTagsMultiple(tags []string) ([]string, error) {
	if len(tags) == 0 {
		return nil, nil
	}

	// Build query to find object_ids that have ALL specified tags
