...
func TestApplyAddBulk_CreateHeading(t *testing.T) {
	t.Parallel()
	v := testutil.NewTestVault(t).
		WithSchema(testutil.PersonProjectSchema()).
		WithFile("projects/with-log.md", "---\ntype: project\ntitle: With Log\nstatus: active\n---\n\n### Log\n\n- old entry\n").
		WithFile("projects/without-log.md", "---\ntype: project\ntitle: Without Log\nstatus: active\n---\n\n## Notes\n").
		Build()
	...
	req := AddBulkRequest{
		VaultPath:     v.Path,
		VaultConfig:   vaultCfg,
		ObjectIDs:     []string{"projects/with-log", "projects/without-log"},
		Line:          "- new entry",
		HeadingSpec:   "### Log",
		CreateHeading: true,
	}

	preview, err := PreviewAddBulk(req)
	if err != nil {
		t.Fatalf("PreviewAddBulk() error = %v", err)
	}
	if len(preview.Items) != 2 || len(preview.Skipped) != 0 {
		t.Fatalf("preview = %#v, want 2 items and no skips", preview)
	}
	if !strings.Contains(preview.Items[1].Details, "create heading") {
		t.Fatalf("preview details = %q, want create-heading note", preview.Items[1].Details)
	}

	summary, err := ApplyAddBulk(req, nil)
	if err != nil {
		t.Fatalf("ApplyAddBulk() error = %v", err)
	}
	if summary.Added != 2 || summary.Errors != 0 {
		t.Fatalf("summary = %#v, want added=2 errors=0", summary)
	}

	withLog := v.ReadFile("projects/with-log.md")
	if got := strings.Count(withLog, "### Log"); got != 1 {
		t.Fatalf("expected existing Log heading reused, got %d headings:\n%s", got, withLog)
	}
	if !strings.Contains(withLog, "- new entry") {
		t.Fatalf("entry missing in with-log:\n%s", withLog)
	}
	withoutLog := v.ReadFile("projects/without-log.md")
	if !strings.Contains(withoutLog, "### Log\n- new entry") {
		t.Fatalf("heading not created in without-log:\n%s", withoutLog)
	}
}
