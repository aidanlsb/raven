...
func (s *Server) callDirectLast(args map[string]interface{}) (string, bool) {
	vaultPath, err := s.resolveVaultPath()
	if err != nil {
		return errorEnvelope("VAULT_RESOLUTION_FAILED", "failed to resolve active vault", err.Error(), nil), true
	}
	normalized := normalizeArgs(args)
	numberArgs := parseDirectLastNums(normalized["nums"])

	selection, selErr := readsvc.SelectLast(readsvc.SelectLastRequest{
		VaultPath:  vaultPath,
		NumberArgs: numberArgs,
	})
	if selErr != nil {
		var noResults *readsvc.NoLastResultsError
		if errors.As(selErr, &noResults) {
			return errorEnvelope("MISSING_ARGUMENT", "no results available", "Run a query, search, or backlinks command first, then use 'rvn last'", nil), true
		}
		var invalid *readsvc.InvalidSelectionError
		if errors.As(selErr, &invalid) {
			suggestion := ""
			if invalid.Total > 0 {
				suggestion = fmt.Sprintf("Valid range: 1-%d", invalid.Total)
			}
			return errorEnvelope("INVALID_INPUT", invalid.Error(), suggestion, nil), true
		}
		return errorEnvelope("INTERNAL_ERROR", selErr.Error(), "", nil), true
	}

	applyStr := strings.TrimSpace(toString(normalized["apply"]))
	confirm := boolValue(normalized["confirm"])
	if applyStr != "" {
		if selection.Last.Source != lastresults.SourceQuery {
			return errorEnvelope("INVALID_INPUT", "--apply is only supported for query results", "Run 'rvn query --apply ...' for query operations", nil), true
		}

		rt, rtErr := readsvc.NewRuntime(vaultPath, readsvc.RuntimeOptions{OpenDB: false})
		if rtErr != nil {
			return errorEnvelope("CONFIG_INVALID", "failed to load vault config", "Fix raven.yaml and try again", nil), true
		}
		defer rt.Close()

		return s.callDirectLastApply(vaultPath, rt, selection, strings.Fields(applyStr), confirm)
	}

	items := make([]map[string]interface{}, len(selection.Selected))
	for i, e := range selection.Selected {
		items[i] = map[string]interface{}{
			"num":      selection.Numbers[i],
			"id":       e.GetID(),
			"kind":     e.GetKind(),
	...
	if len(numberArgs) == 0 {
		return successEnvelope(map[string]interface{}{
			"source":    selection.Last.Source,
			"query":     selection.Last.Query,
			"target":    selection.Last.Target,
			"timestamp": selection.Last.Timestamp,
			"items":     items,
		}, nil), false
	}

	return successEnvelope(map[string]interface{}{"selected": items}, nil), false
}

func (s *Server) callDirectLastApply(vaultPath string, rt *readsvc.Runtime, selection *readsvc.SelectLastResult, applyArgs []string, confirm bool) (string, bool) {
	queryType, err := readsvc.QueryTypeFromLast(selection.Last, selection.Selected)
	if err != nil {
		return errorEnvelope("INVALID_INPUT", err.Error(), "", nil), true
	}

	execResult := &readsvc.ExecuteQueryResult{QueryType: queryType}
	if queryType == "trait" {
		execResult.Traits = convertLastResultsToTraits(selection.Selected)
	} else {
		execResult.Objects = convertLastResultsToObjects(selection.Selected)
	}

	return s.callDirectQueryApply(vaultPath, rt, selection.Last.Query, execResult, applyArgs, confirm)
}

func convertLastResultsToObjects(results []model.Result) []model.Object {
	objects := make([]model.Object, 0, len(results))
...
func convertLastResultsToTraits(results []model.Result) []model.Trait {
	traits := make([]model.Trait, 0, len(results))
	for _, result := range results {
		switch row := result.(type) {
		case model.Trait:
...
func parseDirectLastNums(raw interface{}) []string {
	values := stringSliceValues(raw)
