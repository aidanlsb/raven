...
| `markdown_link_to_vault_note` | Inline Markdown link/image targets a `.md` note inside the vault, so Raven does not track it as a reference | Convert it to a Raven object/section wikilink (`[[...]]`) so backlinks and moves track it |
