...
Sections use the bare `section` query root and return heading-derived rows with IDs like `file#slug`. Section rows include `line_start`, direct `line_end`, and `subtree_line_end`; use direct ranges for writes and subtree ranges for read/navigation context. Section queries support `--apply "add <text>"` to append inside each matching section (IDs can also be piped into bulk `add`), and `read` with `sections=true` returns a file's outline directly.
