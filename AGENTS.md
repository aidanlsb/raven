...
- **Go 1.26.1+** — all code in standard Go style
...
- **Go toolchain:** `go.mod` requires `go 1.26.1`. `GOTOOLCHAIN` auto-downloads the pinned toolchain on first `go` invocation, so no manual Go install is needed.
